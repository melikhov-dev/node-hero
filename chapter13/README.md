...
После регистрации выполните следующие действия, чтобы добавить Trace в ваше Node.js-приложение. Это займёт всего минуту — и это шаги, которые вы должны выполнить:

![](NodeHeroEbook-TheComplete-020.png)

Легко, правда? Если все пойдёт хорошо, вы увидите, что подключенный сервис только что начал отправлять данные в Trace:
...
* График пропускной способности показывает количество запросов в минуту (rpm) для категорий кодов состояния (200-299 // 300-399 // 400-499 //> 500). Таким образом, вы можете легко отделить здоровые и проблемные HTTP-запросы в своём приложении.
